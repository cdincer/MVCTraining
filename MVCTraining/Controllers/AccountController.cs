...
namespace MVCTraining.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (user != null)
                    {
                        ...
                        //Check if the password changed if it is, hash it again and send it to DB.
        ...
        public async Task<ActionResult> RegisterUserAsync(RegisterUserModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, FirstName = model.Name, IsActive = true, Registered = DateTime.Now, EmailConfirmed = true };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
